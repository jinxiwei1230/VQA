...
for batch in tqdm.tqdm(data_iter):
    batch_size = batch['raw_frame'].shape[0]  # 使用 batch['raw_frame'] 获取批次大小
    print("batch_size:", batch_size)

    for i in range(batch_size):
        for j in range(frame_num):
            # 获取当前帧并进行处理（使用 batch['raw_frame']）
            image = batch['raw_frame'][i][j].permute(1, 2, 0).numpy()

            # 如果数据范围是 [0, 1]，则转换为 [0, 255]
            if image.max() <= 1.0:
...
for frame_idx in frame_indices:
    # 提取对应帧的原始图像
    image = batch['raw_frame'][0][frame_idx].permute(1, 2, 0).numpy()
