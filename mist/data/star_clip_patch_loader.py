...
class VideoQADataset(Dataset):
    def __init__(
        ...
        """
        self.data = json.load(open(f'../data/datasets/star/STAR_{split}.json'))
        self.dset = 'star'
        ...
        with h5py.File(app_feat_file, 'r') as fp:
            # vids = fp['ids']
            qids = json.load(open('../data/datasets/star/qid.json'))
            feats = fp['features']
            print(feats.shape)  # v_num, clip_num, feat_dim
            for id, (qid, feat) in enumerate(zip(qids, feats)):
                # vid = vid.decode(encoding)
                self.frame_feats[qid[0]] = feat
