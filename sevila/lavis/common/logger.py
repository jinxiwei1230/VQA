...
# 主要用于在分布式深度学习训练过程中记录和管理各种度量值，提供平滑的统计数据，并支持日志的格式化输出。
class SmoothedValue:
...
def setup_logger():
    ...
    print(f"Output directory: {output_dir}")  # 调试输出

    if output_dir is None:
        raise ValueError("Output directory is not set in the registry.")

    run_log = osp.join(registry.get_path("output_dir"), "run.log")
