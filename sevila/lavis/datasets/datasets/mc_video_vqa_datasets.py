...
import random
import pdb
# 这段代码是用于处理 NextQA 数据集 的，它定义了一个名为 MCVideoQADataset 的类，继承自 MultimodalClassificationDataset，
# 并且是用来处理视频问答任务的。它结合了视觉（视频）和文本（问题、选项）数据，最终用于多模态分类问题。
class __DisplMixin:
            ...
            try:
                if "VLEP" in qid:
                    ...
                    if ":" in video_id:
                        ...
                        video_id = video_id[:-3]
                        video_path = (
                            "/nas-hdd/shoubin/videos/tvqa/videos_3fps_with_audio/"
                        )
                    vpath = os.path.join(video_path, video_id + ".mp4")
                else:
                    vpath = os.path.join(self.vis_root, str(ann["video_id"]) + ".mp4")
                    print("^&^^&^&^&^&^&^&^&^&^&")
                    print(str(ann["video_id"]))
                    print(vpath)
                    print("mcmcmcmcmcmcmcmc")
