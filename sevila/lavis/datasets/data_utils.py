...
# 函数load_video和load_video_demo用于加载视频文件，提取指定帧，并按照不同的采样方法（如“均匀采样”或“随机采样”）选择帧。
# 返回的视频帧是一个四维的张量 (C, T, H, W)，表示通道数、帧数、高度和宽度。
# 使用decord库进行视频读取和帧的处理，并支持对视频帧的随机、均匀、以及头尾采样。
...
# move_to_cuda和prepare_sample函数将数据移动到CUDA设备（GPU）上，帮助加速深度学习模型的计算。
...
# reorg_datasets_by_split函数将不同的数据集根据训练集、验证集和测试集进行重新组织。
...
# concat_datasets函数用于将多个数据集合并为一个，主要用于训练集的合并，支持处理普通数据集和流式数据集。
...
# extract_archive函数用于从.tar.gz、.zip等压缩文件中提取内容，支持处理多种类型的压缩文件并将其解压到指定目录。
...
# save_frames_grid函数将视频帧保存为图像，使用torchvision库的make_grid函数将多个帧合成为一个图像网格，最后使用PIL库保存为文件
