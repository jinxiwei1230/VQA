...
class SeViLA(Blip2Base):
    ...
    def __init__(self, img_size=224, drop_path_rate=0, output_dir=None,
        ...
        if 'train_loc' in self.task:
            ...
            with torch.no_grad():
                ...
                with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                    ...
                    frame_prefix_id = torch.repeat_interleave(frame_prefix.input_ids, b*t, 0) # 每帧都有
                    ...
                    input_ids_qa = torch.repeat_interleave(input_tokens_qa.input_ids, t, 0) # 每个问题复制t个给该sample的t帧
                    ...
                    # 查看生成的自然语言文本
                    # predicted_indices = torch.argmax(pred_logits_qa, dim=-1)
                    # predicted_text = [self.t5_tokenizer.decode(idx) for idx in predicted_indices.squeeze().tolist()]
    ...
    def save_keyframes(self, image, frame_idx, output_dir, video_id):
        print("-------------output_dir--------------")
        print(output_dir)
        # 创建关键帧保存目录
        ...
        for i, idx in enumerate(frame_idx):
            try:
                ...
                if frame.dim() == 3:  # (c, h, w)
                    ...
                    # 将 RGB 转换为 BGR 以兼容 OpenCV 保存
                    frame = frame[:, :, [2, 1, 0]]  # RGB -> BGR
                    # 定义保存路径
                    file_path = os.path.join(keyframes_dir, f"frame_{idx}_{i}.png")
                    print("idx:",idx)
                    # 保存帧并检查是否成功
                    success = cv2.imwrite(file_path, frame)
                    if success:
                        print(f"Successfully saved frame {i} to {file_path}")
                    else:
                        print(f"Error: Could not save frame {i} to {file_path}")
                else:
                    print(f"Unexpected frame shape: {frame.shape}")
            except IndexError as e:
                print(f"IndexError: {e} - Invalid frame index {idx}")
...
# 调用5
    ...
    def generate(self,
        ...
        # output_dir = "/home/disk2/dachuang1-23/kafka_result/207/nextqa_infer"
        print("------44--55-----output_dir---------44-----55--:")
        print(output_dir)
        ...
        if 'loc' not in self.task or 'uni_eval' in self.task:
            ...
            with torch.cuda.amp.autocast(enabled=(self.device != torch.device("cpu"))):
                image_embeds = self.ln_vision(self.visual_encoder(image)) # b t, n, c
            _, n, _ = image_embeds.shape
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device) # b t n c
        ...
        # inference with localizer             
            ...
            image = image.reshape(-1, c, h, w)
            with torch.cuda.amp.autocast(enabled=(self.device != torch.device("cpu"))):
                image_embeds = self.visual_encoder(image) # b t, n, c
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                if 'qa_vid' in self.task:
                    ...
                    print("frame_idx:", sorted_frames_idx)
                    select_frames = []
                    for i, fs in enumerate(sorted_frames_idx): 
                        video = []
                        for j, f in enumerate(fs):
                    ...
                    # 保存关键帧
                    print("b:", b)
                    videoid = [id.split('_')[1] for id in qid]
                    for i in range(b):
                        print(videoid[i])
                        ...
                        self.save_keyframes(original_images[i], sorted_frames_idx[i], output_dir, f"{videoid[i]}")
                    ...
                    # for i in range(b):
                    #     self.save_keyframes(select_frames[i], output_dir, f"qqq_batch{i}")
