...
# PerturbedTopK 和 PerturbedTopKFunction：实现了一种带噪声的Top-K选择操作。
# 这种方法在选择Top-K元素时添加了噪声，并且实现了前向和反向传播。这对于模型训练中可能需要处理噪声扰动的场景很有用。
#
# HardTopK：这是一个简单的Top-K选择函数，直接从输入中选择Top-K大的元素的索引。
#
# batched_index_select 和 extract_frames_from_indices：这些辅助函数用于从给定的索引中选择特定的帧，
# 并将它们整理成适当的格式。
#
# ModalityEmbeddingsID 和 ModalityEmbeddings：定义了不同的模态嵌入ID和一个用于生成模态嵌入的模块。
# 这些嵌入用于表示不同类型的模态输入（例如文本查询、视觉嵌入等）。
#
# ATPConfig：这是一个配置类，包含了ATP模型的各种参数，包括编码器和选择器的参数。
#
# ATPEncoder：这是ATP模型的多模态Transformer编码器。它对输入的序列进行编码，并结合了模态嵌入。
#
# TopK_Selector：这是ATP模型的选择器部分。它根据输入的视觉和文本特征计算选择得分，并选择Top-K的帧。
# 这部分包括了对输入的视觉和文本特征的嵌入和编码，以及根据选择得分获取选定帧的逻辑。
