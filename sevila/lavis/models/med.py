...
# 负责将输入的词 ID 转换为嵌入向量，并加上位置和类型嵌入，形成词语、位置、类型等特征的组合表示。
...
# 这是 BERT 模型中的自注意力机制模块，用来计算每个词与其他词之间的注意力权重，并通过这些权重来更新词的表示。
...
# 该类对自注意力机制的输出进行线性变换、dropout（随机失活）和层归一化。它通过添加原始输入张量（input_tensor）实现残差连接，确保模型的稳定性和性能。
...
# 该类封装了自注意力机制和输出部分。包括了常规的自注意力和交叉注意力（用于融合多模态信息，如文本和图像）。
...
# 该类实现了前馈神经网络的第一部分，对隐藏状态进行线性变换并应用激活函数。BERT中的前馈网络是每一层的重要组成部分。
...
# 该类是前馈网络的第二部分，将BertIntermediate的输出进行进一步的线性变换、dropout，并应用层归一化和残差连接。
...
# 该类代表BERT模型中的一个Transformer层。每个层由自注意力、前馈神经网络和（可选的）交叉注意力组成，用于在多模态场景下融合不同模态的数据
...
# 实现 BERT 模型的编码器部分，包括多个 BertLayer
...
# 实现 BERT 模型的主架构，包括嵌入层、编码器和池化层（可选）。
...
# 这个类继承自 BertLMHeadModel，用于从编码器生成任务中解耦解码器的前向逻辑。
...
# 这个类继承自 BertModel 和 BaseEncoder，用于处理编码任务
