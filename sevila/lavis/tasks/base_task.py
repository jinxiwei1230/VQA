...
class BaseTask:
    def __init__(self, **kwargs):
        ...
        self.inst_id_key = "instance_id"  # 有啥用？
...
# 调用3
    def evaluation(self, model, data_loader, output_dir, cuda_enabled=True):
        ...
        for samples in metric_logger.log_every(data_loader, print_freq, header):
            ...
            print("$$$$$$$$$$$$$$samples%%%%%%%%%%%%%%%")
            eval_output = self.valid_step(model=model, samples=samples, output_dir = output_dir)
            print("$$$$$$$$$$$$$$samples@@@@@@@@@@@@@@@@")
    ...
    def _train_inner_loop(
        ...
        metric_logger.add_meter("data_time", SmoothedValue(fmt="{avg:.4f}"))
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            metric_logger.update(data_time=time.time() - start_data_time)
    ...
    def save_result(result, result_dir, filename, remove_duplicate=""):
        ...
        if is_main_process():
            logging.warning(f"rank {get_rank()} starts merging results.")
