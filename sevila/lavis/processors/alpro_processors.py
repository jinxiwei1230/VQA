...
# 一个基础处理器类，用于定义视频数据处理的标准操作。它包括对视频帧的归一化处理，设定默认的均值和标准差，用于处理每帧图像的像素值。
...
# 用于视频训练的处理器，包含随机裁剪、水平翻转、数据增强等操作。它将加载的视频数据进行一系列的转换操作，如调整大小、归一化等。
...
# 用于评估视频的处理器，它更侧重于保持输入视频帧的完整性，只进行一些基本的转换如类型转换、归一化等。
...
# 处理器从指定的视频路径加载视频帧，然后对视频进行一系列预处理步骤，最终返回一个适合输入到模型的视频张量。
# 训练和评估处理器的主要区别在于，训练处理器会对视频进行更复杂的数据增强（如裁剪、翻转），而评估处理器则保持原始视频的结构。