...
# 定义了一个 RunnerBase 类，用于训练和评估机器学习模型。该类支持使用 PyTorch 进行分布式训练，并包含处理不同数据集、训练配置和模型检查点的功能。
...
class RunnerBase:
    ...
    def dataloaders(self) -> dict:
        ...
        if self._dataloaders is None:
            ...
            datasets = reorg_datasets_by_split(
                self.datasets
            )  # 按划分重新组织datasets结构，由按数据集换为按train/val/test
            self.datasets = concat_datasets(datasets)  # 把训练集变为ConcatDataset
                ...
                for split_name in self.valid_splits:
                    if split_name == "val":
                        self._save_checkpoint(
                            cur_epoch, is_best=False, remove_last=True
                        )
                    logging.info(f"Evaluating on {split_name}.")

                    print("val_log11111111111111111111")
    ...
    # 调用1
    def evaluate(self, output_dir, cur_epoch="best", skip_reload=False):
        test_logs = dict()

        if len(self.test_splits) > 0:
            for split_name in self.test_splits:
                print("test_logs2222222222222222222222222")
                test_logs[split_name] = self.eval_epoch(
                    split_name=split_name, cur_epoch=cur_epoch, skip_reload=skip_reload, output_dir = output_dir
                )
    ...
    def eval_epoch(self, split_name, cur_epoch, output_dir, skip_reload=False):
        ...
        print("____________eval_epoch_______________")
        results = self.task.evaluation(model, data_loader, output_dir)
    ...
    @main_process
    def _save_checkpoint(self, cur_epoch, is_best=False, remove_last=False):
        """
        Save the checkpoint at the current epoch.
        """
        # 删掉上一个epoch的checkpoint
        if remove_last and cur_epoch > 0:
            last_ckpt_path = os.path.join(
                self.output_dir,
                f"checkpoint_{cur_epoch - 1}.pth",
            )
            if os.path.exists(last_ckpt_path):
                logging.info(f"Removing checkpoint {last_ckpt_path}.")
