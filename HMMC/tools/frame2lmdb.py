...
def _transform(n_px):  #定义了视频帧的预处理流程，包括调整大小、中心裁剪、颜色转换（RGB）、归一化等操作
    return Compose([
...
def video_to_tensor(video_file, num_frames=24):  # 从输入视频文件中采样固定数量的帧（默认 24 帧）
    # time1 = time.time()
...
def get_video_data_list(line):  # 从 JSON 文件的每一行提取 video_id 和视频帧数据
    item = json.loads(line)
    filename = item['video_id'] + '.avi'
    video_path = os.path.join(video_dir, filename)
    video_data_list = video_to_tensor(video_path, num_frames=max_frames)  # 使用 video_to_tensor 采样视频帧
    if video_data_list is None:
...
def main(targetDir, source_json_path, n_size, num_workers):  # 数据写入 LMDB
...
video_dir = "/home/disk2/four_avi"  # video directory
max_frames = 30  # sampling frame nums of a video
lmdb_path = "/home/disk2/DATA/lmdb"  # lmdb files will be created in this path
source_json_path = "/home/disk2/dachuang1-23/HMMC/tools/hmmc.json"  # specify the video_ids(video name) in video dir you want to write to lmdb

if __name__ == "__main__":
    main(targetDir=lmdb_path, source_json_path="/home/disk2/dachuang1-23/HMMC/tools/hmmc.json", n_size=50, num_workers=5)
