...
def compute_metrics(x, log=False):
    ...
    # sx = np.sort(-x, axis=1)
    # if x.ndim == 1:
    #     x = np.expand_dims(x, axis=1)  # 将 x 转换为形状为 (n, 1) 的二维数组
    ...
    metrics = {}
    if len(ind) > 0:
        metrics['R1'] = float(np.sum(ind == 0)) * 100 / len(ind)  # R1
        metrics['R5'] = float(np.sum(ind < 5)) * 100 / len(ind)  # R5
        metrics['R10'] = float(np.sum(ind < 10)) * 100 / len(ind)  # R10
        metrics['MR'] = np.median(ind) + 1  # Median rank
        metrics["MeanR"] = np.mean(ind) + 1  # Mean rank
    else:
        # 如果 ind 为空，设置这些指标为 0 或其他默认值
        metrics['R1'] = 0
        metrics['R5'] = 0
        metrics['R10'] = 0
        metrics['MR'] = 0
...
def logging_rank(sim_matrix, multi_sentence_, cut_off_points_, logger):
    ...
    if len(sim_matrix.shape) == 1:
        sim_matrix = sim_matrix.reshape(-1, 1)  # 确保为 10 x 1
