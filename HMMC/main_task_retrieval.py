...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--batch_size_val', type=int, default=300, help='batch size eval')
...
def eval_epoch(args, model, test_dataloader, device, n_gpu, id, userid):
    ...
    # 定义文件保存路径
    save_dir = "/home/disk2/four_avi/saved_features"
    ...
    if os.path.exists(save_path):
        ...
        batch_visual_output_list = data["batch_visual_output_list"]
        batch_frame_output_list = data["batch_frame_output_list"]

        print("处理 query 和 title 特征")
        batch_query_output_list, batch_title_output_list = [], []
        # 预计算第一个 query_output，只计算一次
        ...
        with torch.no_grad():
            ...
            for bid, batch in enumerate(test_dataloader):
                ...
                if multi_sentence_:
                    ...
                    # 只在第一次计算 query_output
                    if first_query_output is None:
                        first_query_output = model.text_encoder(query_ids[:1], query_mask[:1])  # 仅计算一次
                    # 复制第一个 query_output，匹配 batch 大小
                    print("len(query_ids):", len(query_ids))
                    query_output = first_query_output.repeat(len(query_ids), 1)
                    batch_query_output_list.append(query_output)
                    title_output = torch.zeros_like(first_query_output).repeat(len(query_ids), 1)
                    batch_title_output_list.append(title_output)
                    print("multi_sentence----------------end")
                ...
                # 对于单一文本检索任务，直接提取文本和视频的特征。
                    ...
                    if args.task == "retrieval_VT":
                        ...
                        title_output = torch.zeros_like(
                            query_output)  # 标题的输出 title_output 会被设为与查询输出 (query_output) 相同形状的全零张量
                    else:
                        raise ValueError("wrong task type:{}".format(args.task))
        ...
        torch.save({
            ...
            "batch_frame_output_list": batch_frame_output_list
        }, save_path)
    ...
    # 获取相似度最高的5个视频的索引
    top_k_indices = np.argsort(first_text_similarities)[-5:][::-1]  # 排序并取前5个最大值
    print("top_k_indices:", top_k_indices)
    # 获取与第一个文本相似度最高的5个视频ID
    top_k_indices = [video_ids[i] for i in top_k_indices]
    print("与第文本相似度最高的5个视频ID:", top_k_indices)
    # 将索引保存到文件
